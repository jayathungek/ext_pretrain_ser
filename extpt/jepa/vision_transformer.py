...
try: 
    from extpt.jepa.patch_embed import PatchEmbed, PatchEmbed3D
    from extpt.jepa.modules import Block
    from extpt.jepa.pos_embs import get_2d_sincos_pos_embed, get_3d_sincos_pos_embed
    from extpt.jepa.tensors import trunc_normal_
    from extpt.jepa.mask_utils import apply_masks
...
def vit_large(patch_size=16, **kwargs):
    model = VisionTransformer(
        patch_size=patch_size, embed_dim=1024, depth=24, num_heads=16, mlp_ratio=4, num_frames=FRAMES,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
...
def load_pretrained_videovit(pretrained_path):
    checkpoint = torch.load(pretrained_path)["encoder"]
    checkpoint = {
    ...
    for k, v in checkpoint.items():
        if not k.startswith("block"):
            clipped_checkpoint[k] = v
        else:
            if int(k.split(".")[1]) < JEPA_PRETRAINED_CUTOFF:
                clipped_checkpoint[k] = v
    vitbase = vit_large()
    del vitbase.blocks[JEPA_PRETRAINED_CUTOFF:]
    vitbase.load_state_dict(clipped_checkpoint)
