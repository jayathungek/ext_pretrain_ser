...
import random
from extpt.constants import ATTN_DROPOUT
...
class ASTModel(nn.Module):
    def __init__(self, label_dim=527,
                 ...
                 input_fdim=128, input_tdim=1024, emb_dim=1024,
        ...
        if pretrain_stage == True:
            ...
            self.v = timm.create_model('vit_deit_base_distilled_patch16_384', pretrained=False, attn_drop_rate=ATTN_DROPOUT)
        ...
        # use a pretrained models for finetuning
            ...
            # if load_pretrained_mdl_path is None ^ pt_state_dict is None:
            #     raise ValueError('Please set either load_pretrained_mdl_path or pt_state_dict to load a pretrained model.')
                
            if load_pretrained_mdl_path is not None:
                sd = torch.load(load_pretrained_mdl_path, map_location=device)
            elif pt_state_dict is not None:
                sd = pt_state_dict
            else:
                raise ValueError('Please set either load_pretrained_mdl_path or pt_state_dict to load a pretrained model.')
            ...
            try:
                ...
                # p_input_fdim, p_input_tdim = 128, 700   # iemocap
                # p_input_fdim, p_input_tdim = 128, 200   # tess
                # p_input_fdim, p_input_tdim = 128, 1024  # original
                # p_input_fdim, p_input_tdim = 128, 688   # mspodcast
                p_input_fdim, p_input_tdim = 128, emb_dim
            except:
            ...
            print('now load a SSL pretrained models from ' + load_pretrained_mdl_path if load_pretrained_mdl_path else 'pt_only_ast')
...
if __name__ == '__main__':
    ...
    ast_mdl = ASTModel(
                 ...
                 pretrain_stage=True, load_pretrained_mdl_path=f"saved_models/SSAST-Base-Patch-400.pth")
